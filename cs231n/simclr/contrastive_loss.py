...
def sim(z_i, z_j):
    ...
    norm_dot_product = (z_i / torch.linalg.norm(z_i)).dot(z_j / torch.linalg.norm(z_j))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k = -torch.log(
            torch.exp(sim(z_k, z_k_N) / tau) / torch.sum(torch.exp(sims_k / tau))
        )
        l_k_N = -torch.log(
            torch.exp(sim(z_k_N, z_k) / tau) / torch.sum(torch.exp(sims_k_N / tau))
        )
